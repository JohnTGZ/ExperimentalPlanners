#include "global_planner/Dijkstra2.h"
#include <pluginlib/class_list_macros.h>

PLUGINLIB_EXPORT_CLASS(global_planner::Dijkstra2, nav_core::BaseGlobalPlanner) //register this planner as a BaseGlobalPlanner plugin
 namespace global_planner
{

Dijkstra2::Dijkstra2(): costmap_(NULL), initialized_(false), allow_unknown_(true) {}

Dijkstra2::Dijkstra2(std::string name, costmap_2d::Costmap2DROS *costmap_ros): costmap_(NULL), initialized_(false), allow_unknown_(true)
{
    initialize(name, costmap_ros);
}

Dijkstra2::Dijkstra2(std::string name, costmap_2d::Costmap2D* costmap, std::string global_frame): costmap_(NULL), initialized_(false), allow_unknown_(true)
{
    initialize(name, costmap, global_frame);
} 

void Dijkstra2::initialize(std::string name, costmap_2d::Costmap2DROS *costmap_ros)
{
    costmap_ros_ = costmap_ros;
    initialize(name, costmap_ros->getCostmap(), costmap_ros->getGlobalFrameID());
}

void Dijkstra2::initialize(std::string name, costmap_2d::Costmap2D *costmap, std::string global_frame)
{
    if (!initialized_)
    {
        ...
        make_plan_srv_ = private_nh.advertiseService("make_plan", &Dijkstra2::makePlanService, this);
...
bool Dijkstra2::makePlan(const geometry_msgs::PoseStamped& start, 
...
bool Dijkstra2::makePlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal, double tolerance, std::vector<geometry_msgs::PoseStamped>& plan)
{
    if(!initialized_)
    {
...
void Dijkstra2::setCostmap(costmap_2d::Costmap2DROS * costmap_ros)
{
...
void Dijkstra2::MaxTime(double ttrack)
{   
    if (ttrack > max_time)
    {
        max_time = ttrack;
...
void Dijkstra2::addToOpenList(int &current_idx, int &nb_idx)
{
    if(!isFree(nb_idx))
        return;
...
int Dijkstra2::costState(const int &idx)
{
    if (idx <= 0 || idx >= ns_ )
...
void Dijkstra2::updateVertex(int &current_idx, int &nb_idx)
{
    // float old_cost = g_costs[nb_idx];

    float tg_cost = g_costs[current_idx] +  calculatedist(current_idx, nb_idx );

    if (tg_cost < g_costs[nb_idx])
    {
        g_costs[nb_idx] = tg_cost; 
        parents[nb_idx] = current_idx; 
        openList.push_back(Cell(nb_idx, g_costs[nb_idx]) ); 
...
void Dijkstra2::saveCostmapProperties(costmap_2d::Costmap2D* costmap)
{
...
void Dijkstra2::clearRobotCell(unsigned int mx, unsigned int my){
    if(!initialized_){
...
void Dijkstra2::mapToWorld(double &wx, double &wy, const int &mx, const int &my)
{
    wx = (mx * res_);
    wx = ox_ + wx;
    wy = (my * res_);
...
int Dijkstra2::mapToIndex(const int &mx, const int &my)
{
    return mx + (my * nx_) ;
}

void Dijkstra2::indexToMap(const int &idx, int &mx, int &my)
{
    my = idx/nx_; //floor
    // mx = (my * nx_); 
    mx = idx - my * nx_;
...
geometry_msgs::PoseStamped Dijkstra2::indexToPose(const int idx, ros::Time plan_time)
{
...
std::vector<int> Dijkstra2::getNeighbors(const int &current_idx)
{
...
float Dijkstra2::calculatedist(const int &current_idx, const int &nb_idx)
{   
    int mx1; int my1;
    int mx2; int my2;
...
bool Dijkstra2::makePlanService(nav_msgs::GetPlan::Request& req, nav_msgs::GetPlan::Response& resp) {
...
void Dijkstra2::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path)
{
    if(!initialized_)
    {
        ROS_ERROR("This planner has not been initialized yet, but it is being used, please call initialize() before use");
...
bool Dijkstra2::extractGradientPath(std::vector<double> potential, double start_x, double start_y, double goal_x, double goal_y, std::vector<std::pair<float, float> >& path) {
...
float Dijkstra2::gradCell(std::vector<double> potential, int n) 
{
...
void Dijkstra2::publishPotential(std::vector<double> cost_array)
{

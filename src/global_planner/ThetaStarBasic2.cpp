#include "global_planner/ThetaStarBasic2.h"
#include <pluginlib/class_list_macros.h>

PLUGINLIB_EXPORT_CLASS(global_planner::ThetaStarBasic2, nav_core::BaseGlobalPlanner) //register this planner as a BaseGlobalPlanner plugin
 
namespace global_planner
{

ThetaStarBasic2::ThetaStarBasic2(): costmap_(NULL), initialized_(false), allow_unknown_(true) {}

ThetaStarBasic2::ThetaStarBasic2(std::string name, costmap_2d::Costmap2DROS *costmap_ros): costmap_(NULL), initialized_(false), allow_unknown_(true)
{
    initialize(name, costmap_ros);
}

ThetaStarBasic2::ThetaStarBasic2(std::string name, costmap_2d::Costmap2D* costmap, std::string global_frame): costmap_(NULL), initialized_(false), allow_unknown_(true)
{
    initialize(name, costmap, global_frame);
} 

void ThetaStarBasic2::initialize(std::string name, costmap_2d::Costmap2DROS *costmap_ros)
{
    costmap_ros_ = costmap_ros;
    initialize(name, costmap_ros->getCostmap(), costmap_ros->getGlobalFrameID());
}

void ThetaStarBasic2::initialize(std::string name, costmap_2d::Costmap2D *costmap, std::string global_frame)
{
    if (!initialized_)
    {
        ...
        make_plan_srv_ = private_nh.advertiseService("make_plan", &ThetaStarBasic2::makePlanService, this);
...
bool ThetaStarBasic2::makePlan(const geometry_msgs::PoseStamped& start, 
...
bool ThetaStarBasic2::makePlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal, double tolerance, std::vector<geometry_msgs::PoseStamped>& plan)
{
    if(!initialized_)
    {
...
void ThetaStarBasic2::setCostmap(costmap_2d::Costmap2DROS * costmap_ros)
{
...
void ThetaStarBasic2::MaxTime(double ttrack)
{   
    if (ttrack > max_time)
    {
        max_time = ttrack;
...
bool ThetaStarBasic2::isFree(int &idx)
{
...
void ThetaStarBasic2::addToOpenList(int &current_idx, int &nb_idx)
{
    if(!isFree(nb_idx))
        return;
...
int ThetaStarBasic2::costState(const int &idx)
{
    if (idx <= 0 || idx >= ns_ )
...
void ThetaStarBasic2::updateVertex(int &current_idx, int &nb_idx)
{
...
bool ThetaStarBasic2::lineOfSight(int &idx1, int &idx2)
{
    int sx1, sy1, sx2, sy2;
    int x0, x1, y1, y0;
...
bool ThetaStarBasic2::isCheap(int mx, int my)
{
    int idx = mx + my * nx_; 
    if (idx <= 0 || idx >= ns_) 
        return false;
...
void ThetaStarBasic2::saveCostmapProperties(costmap_2d::Costmap2D* costmap)
{
...
void ThetaStarBasic2::clearRobotCell(unsigned int mx, unsigned int my){
    if(!initialized_){
...
void ThetaStarBasic2::mapToWorld(double &wx, double &wy, const int &mx, const int &my)
{
    wx = (mx * res_);
    wx = ox_ + wx;
    wy = (my * res_);
...
int ThetaStarBasic2::mapToIndex(const int &mx, const int &my)
{
    return mx + (my * nx_) ;
}

void ThetaStarBasic2::indexToMap(const int &idx, int &mx, int &my)
{
    my = idx/nx_; //floor
    // mx = (my * nx_); 
    mx = idx - my * nx_;
...
geometry_msgs::PoseStamped ThetaStarBasic2::indexToPose(const int idx, ros::Time plan_time)
{
...
std::vector<int> ThetaStarBasic2::getNeighbors(const int &current_idx)
{
...
float ThetaStarBasic2::calculatedist(const int &current_idx, const int &nb_idx)
{   
    int mx1; int my1;
    int mx2; int my2;
...
float ThetaStarBasic2::calculatehcost(const int &current_idx)
{   
    int mx; int my;
...
bool ThetaStarBasic2::makePlanService(nav_msgs::GetPlan::Request& req, nav_msgs::GetPlan::Response& resp) {
...
void ThetaStarBasic2::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path)
{
    if(!initialized_)
    {
        ROS_ERROR("This planner has not been initialized yet, but it is being used, please call initialize() before use");
...
bool ThetaStarBasic2::extractGradientPath(std::vector<double> potential, double start_x, double start_y, double goal_x, double goal_y, std::vector<std::pair<float, float> >& path) {
    ...
    while (c++<ns*4) {
        ...
        if (potential[stc] >= MAX_COST || potential[stc + 1] >= MAX_COST || potential[stc - 1] >= MAX_COST
                || potential[stcnx] >= MAX_COST || potential[stcnx + 1] >= MAX_COST || potential[stcnx - 1] >= MAX_COST
                || potential[stcpx] >= MAX_COST || potential[stcpx + 1] >= MAX_COST || potential[stcpx - 1] >= MAX_COST
            ...
            if (potential[stc] >= MAX_COST) { //surrounded by obstacles
...
float ThetaStarBasic2::gradCell(std::vector<double> potential, int n) 
{
...
void ThetaStarBasic2::publishPotential(std::vector<double> cost_array)
{

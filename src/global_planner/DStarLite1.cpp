#include <global_planner/DStarLite1.h>
#include <pluginlib/class_list_macros.h>

PLUGINLIB_EXPORT_CLASS(global_planner::DStarLite1, nav_core::BaseGlobalPlanner)
...
DStarLite1::DStarLite1() {}

DStarLite1::DStarLite1(std::string name, costmap_2d::Costmap2DROS *costmap_ros)
{
    initialize(name, costmap_ros);
}

DStarLite1::DStarLite1(std::string name, costmap_2d::Costmap2D* costmap, std::string global_frame)
{
    initialize(name, costmap, global_frame);
} 

void DStarLite1::initialize(std::string name, costmap_2d::Costmap2DROS *costmap_ros)
{
    costmap_ros_ = costmap_ros;
    initialize(name, costmap_ros->getCostmap(), costmap_ros->getGlobalFrameID());
}

void DStarLite1::initialize(std::string name, costmap_2d::Costmap2D *costmap, std::string global_frame)
{
  if (!initialized_)
  {
...
bool DStarLite1::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan)
{
  if(!initialized_)
  {
...
void DStarLite1::saveCostmapProperties()
{
...
void DStarLite1::setGoal(const State &goal)
{
...
std::vector<State> DStarLite1::calcPath()
{
...
bool DStarLite1::computeShortestPath()
{
...
ValuePair DStarLite1::calcKey(const IState &s)
{
  float sv = smaller(getg(s), getRhs(s));
  return ValuePair(sv + calcHCost(s) + km_, sv);
}

void DStarLite1::UpdateVertex(IState u)
{
  if (u != goal)
  {
    float min = MAX_COST;
...
vector<IState> DStarLite1::simpleNeighbors(IState s)
{
  vector<IState> n;
  if (isFree(s.x + 1, s.y))
...
vector<IState> DStarLite1::neighbors(IState s)
{
  vector<IState> n;
  if (!isFree(s))
    return n;
...
int DStarLite1::equivalent(int v)
{
  if (v < costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
  {
    v = COST_NEUTRAL + COST_FACTOR * v;
...
bool DStarLite1::inUList(IState s)
{
  return ulisthash.find(s) != ulisthash.end();
}

void DStarLite1::removeUList(IState s)
{
...
void DStarLite1::insertUList(ValuePair total, IState s)
{
  DQPair q(total, s);
  ulist.push(q);
  ulisthash.insert(s);
...
float DStarLite1::getRhs(const IState &s)
{
  // if (s == goal)
  //   return 0;
...
float DStarLite1::getg(const IState &s)
{
...
int DStarLite1::costState(const IState &s)
{
...
int DStarLite1::costState(unsigned int x, unsigned int y)
{
  if (outOfBound(x, y))
    return costmap_2d::NO_INFORMATION;
...
float DStarLite1::distToStart(const IState &s)
{
  float dist = sqrt((start.x - s.x)*(start.x - s.x) + (start.y - s.y)*(start.y - s.y));
  return dist;
}

float DStarLite1::calcHCost(const IState &s)
{
  float d = distToStart(s);
  return d * COST_NEUTRAL;
}

float DStarLite1::DistState(IState p, IState t) {
    return sqrt((p.x-t.x)*(p.x-t.x) + (p.y-t.y)*(p.y-t.y));
}

float DStarLite1::calcDist(IState p, IState t)
{
  return DistState(p, t) * costState(t);
}

float DStarLite1::DistConnNeighbour(IState p, IState t) {
  int dx = abs(p.x - t.x);
  int dy = abs(p.y - t.y);
...
float DStarLite1::smaller(float x, float y)
{
  if (x < y)
    return x;
  return y;
...
bool DStarLite1::different(float x, float y)
{
  if (isinf(x) && isinf(y))
    return false;
  return (fabs(x - y) > TOL);
...
bool DStarLite1::isFree(const IState &s)
{
...
bool DStarLite1::isFree(int x, int y)
{
  return isFree(IState(x, y));
}

bool DStarLite1::isCheap(int x, int y)
{
  return isCheaper(x, y, cheap_cost);
}

bool DStarLite1::isCheaper(int x, int y, float thres)
{
  if (outOfBound(x, y))
    return false;
...
bool DStarLite1::outOfBound(int x, int y)
{
  return(x <0 || x > nx || y <0 || y > ny);
}

bool DStarLite1::isValid(DQPair qp)
{
...
bool DStarLite1::lineOfSight(const IState &s, const IState &n)
{
...
std::vector<State> DStarLite1::extractPath_parents()
{
 ...
 */
std::vector<State> DStarLite1::extractPath_min()
{
  IState current = this->start;
...
vector<IState> DStarLite1::smooth(vector<IState> orig)
{
...
void DStarLite1::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path)
{
    if(!initialized_)
    {
...
void DStarLite1::publishPotential()
{

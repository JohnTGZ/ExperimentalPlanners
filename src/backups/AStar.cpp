...
PLUGINLIB_EXPORT_CLASS(global_planner::AStar, nav_core::BaseGlobalPlanner) //register this planner as a BaseGlobalPlanner plugin
 namespace global_planner
{

AStar::AStar(): costmap_(NULL), initialized_(false), allow_unknown_(true) {}

AStar::AStar(std::string name, costmap_2d::Costmap2DROS *costmap_ros): costmap_(NULL), initialized_(false), allow_unknown_(true)
{
    initialize(name, costmap_ros);
}

AStar::AStar(std::string name, costmap_2d::Costmap2D* costmap, std::string global_frame): costmap_(NULL), initialized_(false), allow_unknown_(true)
{
...
void AStar::initialize(std::string name, costmap_2d::Costmap2D *costmap, std::string global_frame)
{
    if (!initialized_)
    {
        ...
        make_plan_srv_ = private_nh.advertiseService("make_plan", &AStar::makePlanService, this);
...
bool AStar::makePlan(const geometry_msgs::PoseStamped& start, 
...
bool AStar::makePlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal, double tolerance, std::vector<geometry_msgs::PoseStamped>& plan)
{
    if(!initialized_)
    {
...
void AStar::setCostmap(costmap_2d::Costmap2DROS * costmap_ros)
{
...
void AStar::MaxTime(double ttrack)
{   
    if (ttrack > max_time)
    {
        max_time = ttrack;
...
void AStar::addToOpenList(int &current_idx, int &nb_idx)
{
    if(!isFree(nb_idx))
        return;
    ...
    if(openhash.find(nb_idx) == openhash.end())
        g_costs[nb_idx] = MAX_COST;

    updateVertex(current_idx, nb_idx);

}

int AStar::costState(const int &idx)
{
    if (idx <= 0 || idx >= ns_ )
...
void AStar::updateVertex(int &current_idx, int &nb_idx)
{
...
void AStar::removeFromOpenhash(int &nb_idx)
{
...
// bool AStar::lineOfSight(int &idx1, int &idx2)
// {
//     int sx1, sy1, sx2, sy2;
//     int x0, x1, y1, y0;
...
// bool AStar::isCheap(int mx, int my)
// {
//     int idx = mx + my * nx_; 
//     if (idx <= 0 || idx >= ns_) 
//         return false;
...
void AStar::saveCostmapProperties(costmap_2d::Costmap2D* costmap)
{
...
void AStar::mapToWorld(double &wx, double &wy, const int &mx, const int &my)
{
    wx = (mx * res_);
    wx = ox_ + wx;
    wy = (my * res_);
...
int AStar::mapToIndex(const int &mx, const int &my)
{
    return mx + (my * nx_) ;
}

void AStar::indexToMap(const int &idx, int &mx, int &my)
{
    my = idx/nx_; //floor
    // mx = (my * nx_); 
    mx = idx - my * nx_;
...
geometry_msgs::PoseStamped AStar::indexToPose(const int idx, ros::Time plan_time)
{
...
std::vector<int> AStar::getNeighbors(const int &current_idx)
{
...
float AStar::calculatedist(const int &current_idx, const int &nb_idx)
{   
    int mx1; int my1;
    int mx2; int my2;
...
float AStar::calculatehcost(const int &current_idx)
{   
    int mx; int my;
...
void AStar::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path)
{
    if(!initialized_)
    {
        ROS_ERROR("This planner has not been initialized yet, but it is being used, please call initialize() before use");
...
bool AStar::extractGradientPath(std::vector<double> potential, double start_x, double start_y, double goal_x, double goal_y, std::vector<std::pair<float, float> >& path) {
...
float AStar::gradCell(std::vector<double> potential, int n) 
{
...
void AStar::publishPotential(std::vector<double> cost_array)
{

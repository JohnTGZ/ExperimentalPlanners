...
int ThetaBasicSimple::costState(const int &idx)
{
    if (idx <= 0 || idx >= ns_ )
...
bool ThetaBasicSimple::isCheap(int mx, int my)
{
    int idx = mx + my * nx_; 
    if (idx <= 0 || idx >= ns_) 
        return false;
...
int ThetaBasicSimple::mapToIndex(const int &mx, const int &my)
{
    return mx + (my * nx_) ;
}
...
float ThetaBasicSimple::calculatedist(const int &current_idx, const int &nb_idx)
{   
    int mx1; int my1;
    int mx2; int my2;
...
float ThetaBasicSimple::calculatehcost(const int &current_idx)
{   
    int mx; int my;
...
bool ThetaBasicSimple::extractGradientPath(std::vector<double> potential, double start_x, double start_y, double goal_x, double goal_y, std::vector<std::pair<float, float> >& path) {
...
float ThetaBasicSimple::gradCell(std::vector<double> potential, int n) 
{
